...
export async function getDestination(obr: OBR, portal: Item, destinationsCache: Record<string, Vector2>): Promise<Vector2> {
  if (destinationsCache[portal.id]) {
    return Promise.resolve(destinationsCache[portal.id]);
  }

  const items = await obr.scene.items.getItems([portal.metadata[DESTINATION_ID_METADATA_ID] as string]);

...
export default async function handleMovement(obr: Obr, items: Item[]) {
  ...
  items = items.filter(
    ({ layer, metadata }) =>
      layer === "CHARACTER" && !metadata[JUST_TELEPORTED_METADATA_ID],
  );
  if (items.length === 0) {
    return;
  }
...
async function collides(
  ...
  { position: { x, y } }: Item,
  portal: Item,
  boundsCache: Record<string, BoundingBox>,
): Promise<boolean> {
...
async function getBounds(
  ...
  return (boundsCache[item.id] = await obr.scene.items.getItemBounds([
    item.id,
  ]));
}
